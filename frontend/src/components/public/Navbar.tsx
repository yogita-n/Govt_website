    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-darkgreen/95 backdrop-blur shadow-lg' : 'bg-darkgreen'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            {/* Language Toggle — label stays fixed so user always knows what to click */}
            <button
              onClick={toggleLang}
              className="flex items-center gap-1.5 text-white/80 hover:text-white border border-white/30 px-3 py-1.5 rounded-lg text-sm transition-colors"
            >
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[60] bg-darkgreen flex flex-col">
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-6">
            ...
            <button onClick={toggleLang} className="flex items-center gap-2 text-white border border-white/30 px-4 py-2 rounded-lg">
